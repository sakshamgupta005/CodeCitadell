...
class LLMService:
    ...
    async def diagnose_global_issue(
        ...
        instructions = (
            ...
            "  - Set 'investigation_reasoning' to a friendly, natural, direct, and comprehensive explanation of the concept based on the retrieved documentation. Address the user directly in the second person. Do NOT write in the third-person or use robotic reasoning (e.g., do NOT say 'The user is asking...').\n"
            ...
            "  - Set 'investigation_reasoning' to a natural, empathetic, and direct explanation of what might be happening, referencing matching symptoms. Write directly to the user (e.g., 'A mesh node that won't connect is usually caused by...'). Do NOT say 'The user is reporting...' or expose internal robotic reasoning.\n"
            "  - Set 'probable_causes' to likely causes.\n"
            "  - Set 'follow_up_question' to a targeted clarifying question (e.g. status lights, cable state) to begin progressive troubleshooting.\n"
            ...
            "  - Set 'cited_sources' to a list of 1-based indices of the retrieved sources actually used for this (e.g. [1]).\n\n"
            "You MUST return a single, valid JSON object with the keys 'detected_product_id', 'detected_product_name', 'investigation_reasoning', 'probable_causes', 'follow_up_question', 'next_step', 'recommended_action', and 'cited_sources'. Do not wrap the JSON in markdown code blocks. Do not add any text before or after the JSON."
        )
    ...
    def _diagnostic_instructions() -> str:
        return (
            ...
            "   - Set 'probable_causes' to an empty list [].\n"
            "   - Set 'investigation_reasoning' to a friendly, natural, direct, and comprehensive explanation of the concept or product. Address the user directly using the second person (e.g. 'Moss is...'). Do NOT include internal robotic reasoning. Cite the retrieved source documents if they contain relevant info.\n"
            ...
            "   - Follow an investigative troubleshooting process. Do NOT jump to the final answer/diagnosis on the first turn if there is ambiguity.\n"
            "   - If the diagnostic history is empty or short, or if you need more details to confirm the root cause: ask a single, targeted, highly specific elimination question (e.g. 'What color is the status light?', 'Is the ethernet cable plugged in?'). Set 'probable_causes' to the list of possibilities you are currently considering.\n"
            "   - If the diagnostic history has answers, use them to narrow down the 'probable_causes'. If you are now confident in the root cause, present the final diagnosis. Otherwise, ask the next elimination question.\n"
            "   - Set 'investigation_reasoning' to a natural, empathetic, and direct response to the user. Start by explaining the potential causes or introducing the troubleshooting steps. Crucially: DO NOT write in the third-person or expose a chain-of-thought (e.g., do NOT say 'The user is reporting a connection issue. I need to...'). Write directly to the user (e.g., 'A mesh node that won't connect is usually caused by one of three things...').\n"
            ...
            "   - Set 'recommended_action' to a specific fix or check from the documentation.\n"
            "   - Set 'cited_sources' to the 1-based indices of the retrieved sources that you used to construct the answer (e.g., [1] or []).\n\n"
            "You MUST return a single, valid JSON object with the keys 'probable_causes', 'investigation_reasoning', 'follow_up_question', 'next_step', 'recommended_action', and 'cited_sources'.\n"
